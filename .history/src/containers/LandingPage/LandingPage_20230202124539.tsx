                            ...
                            {navigationList.map((label, index) => {
                                return (
                                    <li className={index === isActive ? 'activeClass' : "nav-item "} key={label.id} onClick={() => handleActiveClass(label.id)}>
                                        <NavLink to={label.to} className='nav-link align-middle px-0 px-sm-2 mb-2 mt-2' >
                                            <i className="bi-alarm" style={{ color: "gray" }}></i> <span className="ms-1 d-none d-sm-inline ml-2" >{label.title}</span>
                                        </NavLink>
                                    </li>
                                )
                            })}
